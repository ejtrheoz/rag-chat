...
async def send_message(request: Request, current_user: Users = Depends(get_current_user)):
    ...
    if Preferences.detect_preference(message):
        print("Detected preference")
        preferences = Preferences.get_user_preferences(str(current_user.id), "Get messages where user says what he likes and ignore such messages as 'what i like'", k=100)
